...
4. Project `chat.message` as the canonical redacted WebSocket payload: identifiers, message type, server timestamps, server-authoritative delivery state, and text `content` only. Never include media URLs, `read_receipts`, or sender names.
5. Treat WebSocket delivery as optional projection: REST success is based only on committed PostgreSQL state, and reconnect always reconciles through paginated REST history.
