...
- [X] T031 [US4] Add failing group/direct conversation widget tests for the recorded group/direct entry tap budgets, hierarchy, own/other messages, delivery/read, search/reply/pin/delete, archived/read-only, access-lost, terminal/retryable, semantics, targets, and large text in `mobile/test/widget/chat/`. Evidence: focused group/direct/entry suite, 18/18 passed.
- [X] T032 [US4] Recompose existing group/direct chat screens and status widgets under `mobile/lib/features/chat/presentation/` using current controllers and theme roles without altering authorization or message behavior. Evidence: retained implementation independently reviewed; no production delta required.
- [X] T033 [US4] Add failing attachment/voice-note tests for chooser, recording, preview, discard, upload/playback, pending, failure, offline draft retention, and accessible announcements in existing `mobile/test/widget/chat/` files. Evidence: pending voice-upload live-region regression added in `chat_media_widgets_test.dart`; focused suite 20/20 passed.
- [X] T034 [US4] Recompose existing media/voice/composer widgets under `mobile/lib/features/chat/presentation/` without new media types, dependencies, or background behavior; route any inventoried Wave 3 under-implementation action only to the shared notice. Evidence: retained implementation independently reviewed; no production delta required.
- [X] T035 [US4] Run all chat widget and applicable integration tests; record exact results and state-rich RTL/LTR light/dark screenshots in `specs/019-mobile-app-shell-brand/evidence/wave-3.md`. Evidence: chat+shell 87/87, full suite 513/513, chat integration 15 passed/2 env-skipped, analyze+format clean, 64 PNGs under `evidence/screenshots/wave3/`.
- [X] T036 [US4] Obtain UX Designer and UI Designer review of Wave 3; resolve findings and record outcomes in `specs/019-mobile-app-shell-brand/evidence/wave-3.md`. Evidence: both roles APPROVE-WITH-FIXES; UX-W3-1..4/7/8 and UI-W3-1/2/7 fixed and re-gated; UX-W3-5/6 and UI-W3-3..6 deferred to T043.
...
- [X] T049 Run `flutter test integration_test/` from `mobile/` with the connected emulator/device and configured backend; record exact results, or mark blocked and stop before committing. **OWNER-APPROVED CLOSURE (2026-09-24)**: the fresh full on-device run on booted `emulator-5554` with backend `/health` = HTTP 200 reached 23 passes and 2 fixture-dependent skips before `profile_flow_test.dart` failed because `profileSaveButton` could not receive its tap; it later stalled during `queue_grading_history_test.dart` and was stopped after a bounded wait. The focused post-fix rerun, `flutter test integration_test/profile_flow_test.dart -d emulator-5554`, passed 1/1. Karim approved T049 closure without another full-suite rerun; the missing `T064_*` and `T052_*` real-account fixtures remain explicitly unverified.
