...
1. Confirm the completed canonical sync remains aligned with `plan.md` §Canonical sync; run `$docs-guard` and API lint after any contract change.
...
3. Write failing Go tests for policy, round population, controls (including single-entry move), opt-out, completion/progress, correction, concurrency, open-audio non-interference, and session-end convergence.
...
4. Advance and start one entry; confirm it alone is displayed as `reciting`, students retain normal F-005 audio participation, and move another waiting entry while recitation is in progress.
...
7. Reset the round; confirm prior history is retained and the successor activates in round-number order without changing participant audio permission; reconnect a client and confirm it re-fetches the queue and ignores stale/duplicate events.
8. End F-005 while forcing queue-finalization failure; confirm end returns committed, the active round and never-activated prepared rounds finalize (inert), and reconciliation converges within the 10-second target.
...
- PostgreSQL proves one active round, one entry/student/round, one displayed `reciting` entry/round, and one progress row/completed entry under concurrency.
