...
- This machine has no local Flutter/Dart/Node. Karim pre-authorized the Docker fallback — use it without asking: `docker run --rm -e FLUTTER_SUPPRESS_ANALYTICS=true -v "<repo-root>:/workspace" -v halaqaty-pub-cache:/root/.pub-cache -w /workspace/mobile ghcr.io/cirruslabs/flutter:stable <cmd>` (Flutter 3.44.0 / Dart 3.12.0). Run `flutter pub get` first. Flutter integration tests (ephemeral Linux scaffold + xvfb) and the Spectral Docker command have step-by-step recipes in `docs/engineering/development/LOCAL_ENVIRONMENT_RUNBOOKS.md` — read that section before running either.
