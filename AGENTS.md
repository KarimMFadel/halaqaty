...
- This machine has no local Flutter/Dart/Node. Karim pre-authorized the Docker fallback — use it without asking: `docker run --rm -e FLUTTER_SUPPRESS_ANALYTICS=true -v "<repo-root>:/workspace" -v halaqaty-pub-cache:/root/.pub-cache -w /workspace/mobile ghcr.io/cirruslabs/flutter:stable <cmd>` (Flutter 3.44.0 / Dart 3.12.0; `gmeligio/flutter-web:3.44.9` also present). Run `flutter pub get` first; `build_runner` is currently a no-op (no `@riverpod` annotations). Integration tests: use the locally built image `halaqaty-flutter-ci:local` (cirruslabs/flutter:stable + `clang cmake ninja-build pkg-config libgtk-3-dev libsecret-1-dev xvfb`; rebuild via the Dockerfile in this repo's CI docs if absent), run `flutter create --platforms=linux --project-name halaqaty_mobile .` as an ephemeral scaffold (delete `linux/`, `mobile/.gitignore`, `mobile/README.md`, `lib/main.dart`, `analysis_options.yaml`, `test/widget_test.dart` afterwards), then `xvfb-run -a flutter test integration_test/<file> -d linux` per file, sequentially, up to 3 retries each (batch launches flake on the debug connection). Spectral: `docker run --rm -v "<repo-root>:/workspace" -v halaqaty-npm-cache:/root/.npm -w /workspace node:22-alpine npx --yes @stoplight/spectral-cli lint docs/contracts/openapi.yaml --ruleset .spectral.yaml`.
