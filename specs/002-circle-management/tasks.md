...
- [ ] T029 [P] [US2] Add contract tests for public discovery pagination/filtering and redacted public summaries in `backend/tests/contract/circle_discovery_contract_test.go`.
- [ ] T030 [P] [US2] Add contract tests for public direct join and invite-code join in `backend/tests/contract/circle_join_contract_test.go`.
- [ ] T031 [P] [US2] Add integration tests for duplicate membership, full circle, archived circle, sixth-circle limit, public/private behavior, and atomic join under concurrent requests in `backend/tests/integration/circle_join_test.go`.
- [ ] T032 [P] [US2] Add security/rate-limit tests for unauthenticated discovery/join, invalid credentials, repeated join attempts, and response-safety of public results in `backend/tests/contract/circle_join_security_contract_test.go`.
