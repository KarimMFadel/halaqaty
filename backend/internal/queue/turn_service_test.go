...
func TestTurnServiceCorrectNotesOnlyPreservesGrade(t *testing.T) {
	repo, teacherID, sessionID, students, _, _ := createTurnServiceFixture(t, 1)
	service := NewTurnService(repo)
	_, entry := createTurnServiceActiveRound(t, repo, sessionID, teacherID, students[0], true)
	grade := GradeGood
	...
	correctedNote := "corrected note"
	corrected, err := service.Correct(context.Background(), entry.ID, completed.Version, teacherID, nil, &correctedNote)
	if err != nil {
		t.Fatalf("correct notes only: %v", err)
	}
	if corrected.Grade == nil || *corrected.Grade != grade {
		t.Fatalf("corrected grade = %v, want %s", corrected.Grade, grade)
	}
	if corrected.TeacherNotes == nil || *corrected.TeacherNotes != correctedNote {
		t.Fatalf("corrected notes = %v, want %q", corrected.TeacherNotes, correctedNote)
	}
	progress := progressForEntry(t, repo, entry.ID)
	if progress == nil || progress.Grade == nil || *progress.Grade != grade {
		t.Fatalf("progress after notes-only correction = %+v, want grade %s", progress, grade)
	}
}
